...
// + [createApiMock, RequestsLog] add filter (count, findAll)
...
const createApiMock = async (page, { onGetResponse, allowExternalRequests, isExternalRequest, timeout } = {}) => {

  timeoutMaxDefault = timeout || timeoutMaxDefault;

  if(typeof onGetResponse !== 'function'){
    throw new Error('[createApiMock] findMock must be a function')
  }
  ...
  return { find, waitForRequest, getLog, filter }
};
